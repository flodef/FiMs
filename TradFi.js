...
function init() {
  let tableHTML =
    "<table id=\"addTransactionForm\" class=\"topMenu hidden\"><tr>" +
...
function updateInvestmentTable(id, contents) {
  ...
  for (var i = 0; i < row; ++i) {
    var bgcolor =
      i == row - 1
        ? null
        : contents[i][GLOBAL.tendencyCol].includes("BUY")
          ? "lightgreen"
          : contents[i][GLOBAL.tendencyCol].includes("SELL")
            ? "lightcoral"
            : null;
...
function validateAddForm() {
  ...
  var tOpe = !tType ? name : tQty < 0 ? "SELL" : tQty > 0 ? "BUY" : "DIVIDEND";
  ...
  var errorMsg =
    tQty && isNaN(tQty)
      ...
      : tVal && isNaN(tVal)
        ...
        : tName && !tQty && (!tVal || tVal <= 0)
          ? "Coupon value should be positive."
          : !tName && !tVal
            ? "Approvisionnement/Cost Value should be set."
            : tQty && !tVal
              ? "The Value should be set if the Quantity is set."
              : tUnit && tUnit < 0
                ? "Quantity should have an opposite sign as Value."
                : "";
...
function compareResultData(id, contents) {
  if (contents.length > 1) {
    ...
    for (let i = contents.length - 1; i > 0; --i) {
      // Don't insert the header and reverse loop
      var row = contents[i];
